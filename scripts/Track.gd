...
func emit_wheel_at_head(wheel, extra, is_forward):
	emit_signal("wheel_at_head", wheel, extra, is_forward)

# The wheel has reached the tail
func emit_wheel_at_tail(wheel, extra, is_forward):
...
func _update_crossties():
	...
	for i in range(crosstie_count):
		var t = Transform2D()
		var crosstie_position = curve.samplef((i * crosstie_distance) + crosstie_distance/2.0)
		var next_position = curve.samplef((i + 1) * crosstie_distance)
